...
build_all_status() {
    ...
    done < <(tmux list-panes -a -F "#{pane_id}|#{session_name}|#{window_index}|#{pane_index}|#{@claude_pane_status}|#{@copilot_pane_status}|#{session_attached}|#{session_created}" 2>/dev/null | awk -F'|' '$7>0' | sort -t'|' -k8 -n)
}
